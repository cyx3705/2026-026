...
适用版本：**1.19.0**（宿主 HistoryVulcan **5.1.0**）
...
id 就是模块名（如 `HistoryJanus`），页面集是该模块的全部页。切到别的场景时本模块的页被隐藏，
页面对象不重建，状态都还在。模块想把用户带到自己这里，可以调：

```text
aurora.scene.go id=<模块名>          切到本模块的场景（也认去掉 History 的简称）
aurora.scene.open page=<页面 id>     打开一页：当前场景没有它就切到含它的场景
```

控制台与命令集是常驻页，每个场景都有。`tabTarget=console` 因此在任何场景里都成立。

1.18.4 修掉「中央页拖不动、点不换页」：中央区是一排页签，里面混着命令集（文档身份）与
其余中央页（工具身份），而 AvalonDock 的 `LayoutDocumentPane.IndexOf` 不认工具身份，
于是那一半既换不了页也拖不动。**模块侧不需要改任何声明**——`side=center` 的写法与含义
都没变，模块中央页仍与命令集同处一排页签。

注册协议本来就给不出主文档页：文档身份只属于 Aurora 自己的命令集，
模块声明 `side=center` 拿到的一直是工具窗口，落位在中央区。
