...
面板按钮、命令开关、表格单元格和行内按钮自动显示运行进度：命令期间沿按钮四周显示流动边框，
保留标题与宽度，阻止重复触发；结束后恢复。按钮状态为就绪、运行中、已完成、失败或已取消。
未知总进度时不显示百分比；命令确认保持原策略。面板成功操作会等待数据刷新完成再结束运行态。
`aurora.ui.refreshdata` 现在等待取数完成才返回；原 `PageDataRefresher.Refresh` 保留立即发起语义，
新增 `RefreshAsync` 供需要等待完成的调用方。

描述式页面无需新增按钮字段。直接消费 WPF 组件的调用方，在 RowActionInvoked / CellActionInvoked
事件中同步设置 `e.Completion = ExecuteAsync()`，其中 ExecuteAsync 返回 Task<bool>（true 成功、false 失败/取消）；
未设置则按原同步事件处理。不得 async void 等执行完才设置 Completion。
自建通用按钮可用 AuroraCommandActivity.SetActivity(button, activity)，再调用
activity.RunAsync(() => ExecuteAsync())，统一管理重复触发、异常恢复及运行状态。

响应式 grid 里的表格/泳道按剩余高度分配，窄屏换行后仍可滚动；分支图谱不显示横纵滚动条，
保留拖拽平移与滚轮导航。模块不要另包无限高 StackPanel，也不要用固定表高规避视口布局。

## 表格增量刷新与常驻状态（1.20.4）

表头整行使用控制面板同款圆角底板；不再绘制最右 Padding 空表头，余宽分摊到最右数据列。
刷新保留已有内容；底部常驻显示加载状态、当前条数和最后成功更新时间，错误保持可见直到下一次刷新。
大量行启用虚拟化和回收，同样列不重建，相同行不重复通知；原来的行数组返回值继续支持。

需要非全量取数的页面声明：

```json
{"type":"table","id":"rows","dataSource":{"command":"demo.rows","deltaCommand":"demo.changes","rowKey":"id"},"columns":[{"key":"name","title":"名称","width":"*"}]}
```

首次 `demo.rows` 返回完整快照（字符串字典，可带未展示的唯一 id）：

```json
{"mode":"snapshot","revision":"r1","rows":[{"id":"a","name":"甲"},{"id":"b","name":"乙"}]}
```

之后刷新调用 `demo.changes since=r1`，原 `args` 同样携带；不要在 args 占用保留参数 `since`。

```json
{"mode":"delta","revision":"r2","upserts":[{"id":"a","name":"甲（更新）"},{"id":"c","name":"丙"}],"removes":["b"]}
```

- upserts 按 rowKey 合并字段，省略字段保留；新键追加到末尾。removes 删除指定键，不存在时忽略。
- snapshot 可用于版本过期/重排/重新建立基线。切换取数参数自动回全量，不会混用其它查询的版本。
- 空增量返回空 upserts/removes；revision 为模块自定的不透明字符串，成功应用才推进。
- 所有行必须有非空唯一键；同批重复键、更新与删除冲突、非法载荷整批拒绝，旧内容与版本保持。
- 同表请求串行，重复刷新合并；旧结果不会覆盖新选择。失败显示常驻信息，不清空原内容。
- 旧模块只返回行数组时继续全量取数；Aurora 不会自动把业务扫描改成增量。消费方须实现 deltaCommand。
- 直接引用组件的调用方可在 UI 线程调用 `table.ApplyDelta("id", upserts, removes)`；无需传全部行。
  `table.ShowStatus("正在刷新")` 更新常驻状态。组件数据仅驻留内存，不承诺重启恢复。
...
| `ui.title` | 标签页显示名 |
| `ui.side` | `center` 中央工作区 / `left` / `right` / `bottom` / `tab` 与目标页同一个位置（1.20.2 起没有标签组，一格一页） |
...
**停靠位选择的一条经验**（来自 Janus 的实测教训）：`tabTarget` 指向**别的模块**提供的
窗口是不稳的——模块装载顺序不保证目标先到，没到就按目标的默认方位、再不行按右侧放，
首次启动落位因此随机。要跟随，就跟随宿主自己注册的窗口（如控制台）；
不确定时用 `center`，它不依赖任何模块。

**一格一页**（1.20.2，REQ-UI-100）：每个位置任何时刻只放一页，没有页签、没有页签切换。
`side=tab tabTarget=X` 的意思是「与 X 同一个位置」：登记时 X 占着那一格，新页就藏着，
要它露面走 `aurora.ui.show`（或模块场景的初值——本模块的页进场景时顶掉它位置上的页）。
...
| 位置 | 放什么 |
|---|---|
| `b-Code-Studio/Shell/2-Components/Table/` | 表格与行操作 |
| `b-Code-Studio/Shell/2-Components/Panels/` | 面板与小组件 |
| `b-Code-Studio/Shell/2-Components/Graph/` | 图形类（泳道图等） |
| `b-Code-Studio/Shell/2-Components/Widgets/` | 通用小组件（弹出层、补全输入框、栅格） |
| `b-Code-Studio/Shell/2-Components/Themes/AuroraControls.xaml` | 具名样式 |
...
**一、看得到一份真实的范例。** `b-Code-Studio/Shell/3-HostedPages/Views/HostedPageDescriptions.cs`
