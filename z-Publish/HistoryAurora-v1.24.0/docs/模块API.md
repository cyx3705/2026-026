...
模块版本：**1.24.0**；最低宿主：**HistoryVulcan 5.5.0**
...
（如 `HistoryJanus`，也认去掉 `History` 的简称）。每个页面声明必须写
`scene: "<owner>"`；Aurora 拒绝声明其他模块场景的页面，省略 `scene` 只为旧包兼容并按
`owner` 处理。注册时只以自己的场景决定初始显隐，不能改写其它场景的布局。
...
`kind=content` 的 `options` **选填**（1.24.0，REQ-UI-123）。不给还是旧形状：大段只读等宽正文加一个关闭键。
给了就在正文下方多一组动作候选，按钮也换成「主按钮 + 取消」，结果与 `choice` 一样返回所选 value——
于是「看清正文再决定做什么」可以在同一个窗里完成，不必先弹预览再弹选择。
`options` 写坏了一律拒绝执行，不会退化成没有动作的预览窗。

```
aurora.ui.dialog kind=content title=提交 body="3 个文件变更" content="diff --git ..." ^
  options="[{\"label\":\"提交\",\"value\":\"commit\"},{\"label\":\"丢弃\",\"value\":\"discard\"}]" primary=执行
```
...
页面描述的可选字段 `scene` 取模块 owner。模块的所有页面应显式声明它；不同 owner 的值会被拒绝。
右栏的“页面调整”开关会持久化状态，打开时显示可拖拽的页面标签；按住 Ctrl 仍可临时进入。
