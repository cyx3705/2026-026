...
> **内边距不归页面**：Aurora 在交给停靠层之前统一包一层 8px
> （`Aurora.Space.PageInset`，1.22 由 12 收窄，REQ-UI-047 / 119），数值只有 `PageRegistrar.PagePad` 一处。
> 控制台不经 `Inset` 包边，但取同一个 `PageRegistrar.PageInset`，不再自写 10,8,10,10。
>
...
- **没有独立浮窗**（1.22，REQ-UI-120）：拖出去只是拖动途中的载体，落到停靠点进那一格、落空即隐藏；
  没有指令、菜单或模块能把页面单独浮出来。模块内容不得根据浮动状态再套标题栏。
...
- 卡片圆角 16px、组件圆角 8px、页面内边距 8px，三者满足 16 = 8 + 8（同心）；页签圆角为 5px，窗口外缘为直角。
- 控制台、模块页、自持页的第一个组件到卡片边缘距离相同。
