...
> 1.20.0 改写（REQ-UI-092 的第一步，DEC-032）。顶栏最终要删除，本版先把它的职责转移走。

- 整个 HistoryAurora 的窗口控制组（菜单、最小化、最大化、关闭）在**右栏顶部**，即窗体右上角；
  专注态右栏让位，控制组挂到专注页页头。主文档区的页签行（顶栏）**只留一页**，空白处仍可拖动窗口。
- 工具页顶栏只有页签与专注态的「恢复页面」，没有 ▼ 菜单与 ✕。
- 嵌入页不创建自己的窗口最小化、最大化、关闭或拖动区。
- 页面拖出、停靠走 **Ctrl 标签态**：单独按住 Ctrl 约 0.3 秒，窗格内容换成写着页名的标签，按住拖动，
  落到蓝色停靠点上停靠，落空即隐藏。平时按页签只换页、双击专注；专注和恢复仍由页签与 `aurora.ui.*` 命令管线负责。
