...
internal partial class ShellWindow
{
    ...
    private void RegisterHostedPages()
    {
        var parsed = PageDescriptionReader.Read(
            HostedPages.Views.HostedPageDescriptions.Json,
        ...
        foreach (var page in parsed.Value!.Pages)
        {
            ...
            var built = _hostedPages.Build(HostedPages.Views.HostedPageDescriptions.Owner, page);
            ...
            foreach (var component in built.Missing)
            {
                ...
                _componentRequests?.Record(
                    component,
                    HostedPages.Views.HostedPageDescriptions.Owner,
                    HostedPages.Views.HostedPageDescriptions.Owner + "/" + page.Id,
                    "界面自持页面");
            }
